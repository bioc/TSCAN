...
#'
#' The expected format of \code{pseudotime} is the same as that returned by \code{\link{orderCells}}.
...
.test_multi_pseudotime <- function(x, pseudotime, df, get.lfc, get.spline.coef, trend.only) {
    ...
    for (i in colnames(pseudotime)) {
        ...
        if (get.lfc) {
            ...
            all.lfc[[paste0("logFC.", i)]] <- scuttle::fitLinearModel(all.x[[i]], design=design.lfc)$coefficients[,2]
        }
    }
    ...
    output <- scran::testLinearModel(x2, design=design, contrasts=contrast)
